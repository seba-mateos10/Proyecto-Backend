...
class CartMongo {
  async getAll() {
    try {
      const result = await cartModel
        .find()
        .populate(`${MONGOOSE_DB_COLLECTION_PRODUCTS}.product`)
        .lean();
  ...
  async productAdd(cid, pid) {
    try {
      ...
      const existProduct = exist.products.find(
        (p) => JSON.stringify(p.product._id) === JSON.stringify(pid)
      );

      if (!existProduct) {
        const newProduct = new ProductCart(pid);
        const result = await cartModel
          .findByIdAndUpdate(cid, { $push: { products: newProduct } })
          .populate(`${MONGOOSE_DB_COLLECTION_PRODUCTS}.product`)
          .lean();
        return responseData(200, result);
      }
  ...
  async productRemove(cid, pid) {
    try {
      ...
      const position = exist.products.findIndex(
        (p) => JSON.stringify(p.product._id) === JSON.stringify(pid)
      );
