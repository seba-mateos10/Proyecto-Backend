...
class CartMemory {
  ...
  async getOne(cid) {
    try {
      ...
      const exist = await JSON.parse(result).find(
        (element) => parseInt(element._id) === parseInt(cid)
      );
  ...
  async productAdd(cid, pid) {
    try {
      ...
      let existProduct = await exist.products.find(
  ...
  async productUpdate(cid, pid, obj) {
    try {
      ...
      const existProduct = await exist.products.find(
        (p) => parseInt(p.product) === parseInt(pid)
      );
  ...
  async productRemove(cid, pid) {
    try {
      ...
      const position = await exist.products.findIndex(
        (p) => parseInt(p.product) === parseInt(pid)
      );
