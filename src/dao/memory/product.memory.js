...
class ProductMemory {
  ...
  async getAll(page, limit) {
    try {
      ...
      const nextPage =
        JSON.parse(result).length > page * limit ? page + 1 : null;
      const totalDocs = JSON.parse(result).length;
      const totalPages =
        JSON.parse(result).length / limit < 1 &&
        JSON.parse(result).length / limit !== 0
          ? 1
          : JSON.parse(result).length / limit;
      return responseData(
        200,
        new Paginate(
  ...
  async getOne(pid) {
    try {
      const result = await this.getFile();
      const exist = await JSON.parse(result).find(
        (e) => parseInt(e._id) === parseInt(pid)
      );
  ...
  async create(obj) {
    try {
      if (!(obj.title && obj.code && obj.price && obj.stock && obj.category))
      ...
      const exist = await clone.find(
        (e) => JSON.stringify(e.code) === JSON.stringify(obj.code)
      );
